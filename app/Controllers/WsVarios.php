...
use App\Models\Variables_diversas_model;
use App\Models\Tipo_entidades_model;
...
class WsVarios extends BaseController
{
    ...
    public function datos_configuracion(){
        ...
        $data['datos_configuracion'] = $variables_diversas_model->datos_configuracion(base_url(), date('Y'));
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }
    public function select(){
        $tipo_entidades_model = new Tipo_entidades_model();
        $data['tipo_entidades'] = $tipo_entidades_model->ws_select();
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }
    
    public function select_all(){
        $tipo_entidades_model = new Tipo_entidades_model();
        $data = $tipo_entidades_model->select(3);
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }

    public function ws_select($pagina, $filas_por_pagina, $entidad_id, $tipo_entidad_id){
        $variables_diversas_model = new Variables_diversas_model();
        $entidades_model = new Entidades_model();

        $condicion = array();
        $condicion = ($entidad_id != $variables_diversas_model->param_stand_url) ? array_merge($condicion, array('ent.id' => '='.$entidad_id)) : $condicion;
        $condicion = ($tipo_entidad_id != $variables_diversas_model->param_stand_url) ? array_merge($condicion, array('tip.id' => '='.$tipo_entidad_id)) : $condicion;
                
        $data = $entidades_model->ws_select($pagina, $filas_por_pagina, $condicion, ' ORDER BY ent.id desc');
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }
    
    public function select__(){
